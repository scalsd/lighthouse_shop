            ...
            <div class="block-header">
                <div class="row">
                    <div class="col-lg-6 col-md-8 col-sm-12">
                        <h2><a href="javascript:void(0);" class="btn btn-xs btn-link btn-toggle-fullwidth"><img src="{{asset('backend/assets/images/left-arrow.png')}}" alt="picture" style="width: 15px; height: 15px; padding-top: 2px"></i></a><span class="admin-panel-title">Список издательств</span></h2>
                    </div>            
                </div>
            </div>
                ...
                <div class="col-lg-12">
                    <div class="card" style="background-color:#242424;">
                        <div class="body">
                            <div class="table-responsive">
                                <table class="table table-bordered table-striped table-hover" style="background-color:#d4d3ce; color:#303030">
                                    ...
                                    <tbody>
                                    @foreach($publishing_houses as $item)   
                                        <tr>
                                            ...
                                            <td>
                                                <div style="display: flex; justify-content: center; align-items: center;">
                                                    <a href="{{route('publishing_house.edit',$item->id)}}" class="float-left btn btn-primary btn-sm float-left mr-1" style="height:30px; width:30px;border-radius:50%" data-toggle="tooltip" title="edit" data-placement="bottom"><i class="fas fa-edit"></i></a>
                                                    <form class="float-left ml-1" method="POST" action="{{route('publishing_house.destroy',[$item->id])}}">
                                                    @csrf 
                                                    @method('delete')
                        ...
                        <div class="col-lg-12" style="display: flex; justify-content: flex-end; padding-right: 20px; padding-bottom: 16px;">
                            <button type="button" class="btn btn-outline-secondary" style="background-color: #986f31; color:#303030;" onclick="window.location.href='{{ route('publishing_house.create') }}';">Добавить издательство</button>
                        </div> 
                    </div>                   
                </div>
            </div>
...
@section('scripts')
    ...
    <script>
        $('input[name=toogle]').change(function() {
            ...
            var id=$(this).val();
            $.ajax({
                url:"{{route('publishing_house.status')}}",
                type:"POST",
                data:{
                    _token:'{{csrf_token()}}',
                    mode:mode,
